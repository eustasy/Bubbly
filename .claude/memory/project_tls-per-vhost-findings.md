...
**Consequences, which the shipped config is built around:**

- A Modern/Intermediate _profile_ per site is not achievable on one socket. A site cannot be TLS 1.3-only while its neighbour allows 1.2, and cannot choose its own groups. Genuinely different profiles need separate listen sockets — a distinct IP or port.
- Per-site `ssl_ciphers` does work, but only bites on TLS 1.2, since `ssl_ciphers` never governs TLS 1.3 ciphersuites (those need `ssl_conf_command Ciphersuites=…`, available at the [[nginx-version-floor]] but subject to the same SNI-timing caveat, and unmeasured).
- Because the default server decides, `sites-available/bubbly_default.conf` exists and includes the profile, and site files include it too as a safety net. Before that, the probe found a no-SNI connection served by whichever `sites-enabled` file sorted first — a site's `www`→apex redirect block dictating protocols and groups for the whole machine, changing silently when a site sorting earlier was added.
