...
- `directive/` — snippets meant to be `include`d inside a `server` or `location` block (the TLS profile, OCSP stapling, gzip and Brotli, security headers, logging variants, rate-limit variants, real-ip, request-size limits).
...
- **OCSP stapling ships commented out, in `directive/bubbly_ocsp-stapling.conf`.** Let's Encrypt stopped issuing certificates with an OCSP responder URL on 2025-05-07 and shut its responders down on 2025-08-06, so `ssl_stapling on;` only warns. Verified 2026-09-10 as still publishing one: ZeroSSL, SSL.com, Actalis, DigiCert, GlobalSign, Sectigo — hence an option rather than a deletion. The `resolver` lives in that file too, since stapling is the only thing that needed it.
