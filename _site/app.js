...
(function () {
  ...
  const TEMPLATES = {
    ...
    verifyB:
      "sudo ln -s /etc/nginx/sites-available/{d}_http.conf /etc/nginx/sites-enabled/{d}_http.conf\nsudo nginx -t && sudo service nginx reload",
    ...
    document.querySelectorAll(".b-term .b-copy").forEach(function (btn) {
      ...
      btn.addEventListener("click", async function (e) {
        ...
        try {
          ...
          btn.innerHTML =
            '<svg width="12" height="12" viewBox="0 0 16 16" fill="none" aria-hidden="true"><path d="M3 8.5 L 6.5 12 L 13 4.5" stroke="currentColor" stroke-width="1.6" stroke-linecap="round" stroke-linejoin="round" fill="none"/></svg> Copied';
        } catch {
